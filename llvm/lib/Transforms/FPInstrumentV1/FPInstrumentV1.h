//===-llvm/lib/Transforms/FPInstrumentV1.h  - Interface ----------*- C++ -*-===//
//
//
//
//===----------------------------------------------------------------------===//
...
namespace {
struct FPInstrumentV1 : public ModulePass {
  
public:
  FPInstrumentV1() : ModulePass(ID) {}
  ...
  void handleLoad(Instruction *I, LoadInst *LI, BasicBlock *BB, Function &F, bool flag);
	...
	void handleAlloca(Instruction *I, AllocaInst *A, BasicBlock *BB, Function &F);
  static char ID; // Pass identification, replacement for typeid
private:
	std::queue<StringRef> FunctionList;
  SmallVector<Function*, 8> AllFuncList;
  SmallVector<Value*, 8> MallocList;
  ...
  std::map<Value*, Value*> AddrMap;
//  std::map<Value*, Value*> MallocMap;
  std::map<Instruction*, Instruction*> AllocaMap;

...
ConstantInt* Transformer::getInt32(int n) {
  static llvm::LLVMContext& global = getMyLLVMGlobalContext();
...
bool Transformer::transform(User *it, Value* newTarget, Value* oldTarget, Type* newType, Type* oldType, unsigned alignment) {
...
bool Transformer::transform(LoadInst& inst, Value* newTarget, Value* oldTarget UNUSED,  Type* newType, Type* oldType, unsigned alignment) {
  ...
  if (dyn_cast<PointerType>(newType)) {
    ...
    for (Use &U : loadInst->uses()) {                                                                                          
          User *UR = U.getUser();
      if (CallInst* callInst = dyn_cast<CallInst>(UR)) {
        ...
        bool is_erased = Transformer::transform(UR, newLoad, loadInst, newType, oldType, alignment);
      //  if (!is_erased)
...
bool Transformer::transform(BitCastInst& inst, Value* newTarget, Value* oldTarget, Type* newType, Type* oldType, unsigned alignment) {
  ...
  if (oldBitcast->getType() != newType) {
    ...
    //for(; it != newBitcast->use_end(); it++) {
      ...
      errs() << "\n";
#endif
      bool is_erased = Transformer::transform(UR, newTarget, oldTarget, newType, oldType, alignment);
...
//  for(; it != oldGetElementPtr->use_end(); it++) {
    for (Use &U : oldGetElementPtr->uses()) {
          User *UR = U.getUser();
#if 0
...
//    it->dump();
    ...
    if (dyn_cast<ArrayType>(newType) || dyn_cast<PointerType>(newType)) {
      ...
      is_erased = Transformer::transform(UR, newTarget, oldTarget, temp_newType, temp_oldType, alignment);
    }
    else if (CallInst* callInst = dyn_cast<CallInst>(UR)) {
...
//	      newGlobal->setUnnamedAddr(true); //TODO check later why we need it

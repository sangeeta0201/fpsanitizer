...
bool PInstrument::runOnModule(Module &M) {
    ...
    for (auto &BB : *F) {
      for (auto &I : BB) {
        ...
        if (LoadInst *Load = dyn_cast<LoadInst>(&I)){
          ...
          else if(name == "_ZN2sw4unumplILm64ELm2EEENS0_5positIXT_EXT0_EEERKS3_S5_"){
							AllInsList.push_back(&I);
          }
					else if(name == "_ZN2sw4unummlILm64ELm2EEENS0_5positIXT_EXT0_EEERKS3_S5_"){
							AllInsList.push_back(&I);
					}
					else if(name == "_ZN2sw4unumdvILm64ELm2EEENS0_5positIXT_EXT0_EEERKS3_S5_"){
							AllInsList.push_back(&I);
					}
					else if(name == "_ZN2sw4unummiILm64ELm2EEENS0_5positIXT_EXT0_EEERKS3_S5_"){
							AllInsList.push_back(&I);
					}
  				else{
...
//			else if(ExtractValueInst *EVI = dyn_cast<ExtractValueInst>(&I)){
	...
	//		}
          ...
          if (Callee) {
            ...
            if(name == "_ZN2sw4unum5positILm64ELm2EEaSEd") {
							Value *Addr = CI->getArgOperand(0);
							Value *OP = CI->getArgOperand(1);
  						setReal(&I, Addr, OP, *F); 
            }
            if(name == "_ZN2sw4unumplILm64ELm2EEENS0_5positIXT_EXT0_EEERKS3_S5_"){
  						size_t OpCode = 1; //add
              handleAdd(OpCode, &I, &BB, CI, *F);
            }
						if(name == "_ZN2sw4unummlILm64ELm2EEENS0_5positIXT_EXT0_EEERKS3_S5_"){
  						size_t OpCode = 2; //mul
              handleAdd(OpCode, &I, &BB, CI, *F);
						}
						if(name == "_ZN2sw4unumdvILm64ELm2EEENS0_5positIXT_EXT0_EEERKS3_S5_"){
  						size_t OpCode = 3; //div
              handleAdd(OpCode, &I, &BB, CI, *F);
						}
						if(name == "_ZN2sw4unummiILm64ELm2EEENS0_5positIXT_EXT0_EEERKS3_S5_"){
  						size_t OpCode = 4; //sub
              handleAdd(OpCode, &I, &BB, CI, *F);
						}
						if(name == "_ZN2sw4unum4sqrtILm64ELm2EEENS0_5positIXT_EXT0_EEERKS3_"){
  						size_t OpCode = 5; //sqrt
              handleMath(OpCode, &I, &BB, CI, *F);
						}
            else{
...
void PInstrument::handleFuncMainInit(Function &F){
...
void PInstrument::cleanGEP(StructType *ST,Instruction *I, BasicBlock *BB,  GetElementPtrInst *GEP, Function &F, size_t index) { 
	Instruction *Next = getNextInstruction(I, BB);
	IRBuilder<> IRB(Next);
	//Type* VoidTy = Type::getVoidTy(M->getContext());
...
bool PInstrument::isFPArray(Type *type) {
...
void PInstrument::instrumentAllFunctions(std::string FN) {
	std::ofstream myfile;
...
bool PInstrument::instrumentFunctions(StringRef FN) {
...
void PInstrument::handleMainRet(Instruction *I, Function &F){
  Module *M = F.getParent();
  IRBuilder<> IRB(I);
  Type* VoidTy = Type::getVoidTy(M->getContext());
...
**/
void PInstrument::setReal(Instruction *I, Value *ToAddr, Value *OP, Function &F){
  Module *M = F.getParent();
  IRBuilder<> IRB(I);
...
void PInstrument::handleLLVMMemset(Instruction *I, CallInst *CI, Function &F){
  Module *M = F.getParent();
  IRBuilder<> IRB(I);
...
**/
/*
void PInstrument::setRealCastFToD(Instruction *I, Value *OP, Function &F){
  IRBuilder<> IRB(I);
  Instruction *OpIns = dyn_cast<Instruction>(I->getOperand(0));
...
void PInstrument::handleFree(Instruction *I,  BasicBlock *BB, CallInst *CI, Function &F){
	Module *M = F.getParent();
	Instruction *Next = getNextInstruction(I, BB);
	IRBuilder<> IRB(Next);
  Function *Callee = CI->getCalledFunction();
...
*/
/*
void PInstrument::handleMalloc(Instruction *I, BasicBlock *BB, CallInst *CI, Function &F){
  Module *M = F.getParent();
	Instruction *Next = getNextInstruction(I, BB);
	IRBuilder<> IRB(Next);
  Function *Callee = CI->getCalledFunction();
...
*/
void PInstrument::handleFread(Instruction *I,  BasicBlock *BB, CallInst *CI, Function &F){
	Module *M = F.getParent();
	Instruction *Next = getNextInstruction(I, BB);
	IRBuilder<> IRB(Next);
...
/*
void PInstrument::handleEqOp(Instruction *I,  BasicBlock *BB, CallInst *CI, Function &F){
	Module *M = F.getParent();
	Instruction *Next = getNextInstruction(I, BB);
	IRBuilder<> IRB(Next);
	Value *Addr = CI->getArgOperand(0);
...
*/
void PInstrument::handleCalloc(Instruction *I,  BasicBlock *BB, CallInst *CI, Function &F){
	Module *M = F.getParent();
	Instruction *Next = getNextInstruction(I, BB);
	IRBuilder<> IRB(Next);
...
void PInstrument::handleLLVMMemcpy(Instruction *I, CallInst *CI, Function &F){
  Module *M = F.getParent();
  IRBuilder<> IRB(I);
...
register.
**/
void PInstrument::createPrintFunc(Instruction *I, CallInst *CI, Function &F){
  Module *M = F.getParent();
  IRBuilder<> IRB(I);
  Function *Callee = CI->getCalledFunction();
...
**/
void PInstrument::handleCallInst(Instruction *I, CallInst *CI, BasicBlock *BB, Function &F){
...
void PInstrument::handleFuncInit(Function &F){
  Function::iterator Fit = F.begin();
  BasicBlock &BB = *Fit; 
  BasicBlock::iterator BBit = BB.begin();
  Instruction *First = &*BBit;
...
/*
void PInstrument::handleFuncExit(Instruction *I, ReturnInst *RI, Function &F){
...
void PInstrument::handleLoad(Instruction *I, LoadInst *LI, BasicBlock *BB, Function &F, bool flag){
  Module *M = F.getParent();
  Instruction *Next = getNextInstruction(I, BB);
  IRBuilder<> IRB(Next);
  Type* PtrVoidTy = PointerType::getUnqual(Type::getInt8Ty(M->getContext()));
...
/*
void PInstrument::handleAlloca(Instruction *I, BasicBlock *BB, AllocaInst *A, Function &F){
  Module *M = F.getParent();
...
Instruction* PInstrument::getNextInstruction(Instruction *I, BasicBlock *BB){
  Instruction *Next;
...
void PInstrument::handleMathFunc3Args(Instruction *I, BasicBlock *BB, CallInst *CI, Function &F, int FuncCode){
  if(FuncCode == 0){
    return;
  }
  Instruction *Next = getNextInstruction(I, BB);
...
**/
void PInstrument::handleMathFunc(Instruction *I, BasicBlock *BB, CallInst *CI, Function &F, int FuncCode){
  if(FuncCode == 0){
    return;
  }
  Instruction *Next = getNextInstruction(I, BB);
...
**/
void PInstrument::handleNewPhi(Function &F){
  Module *M = F.getParent();
  Instruction* Next;
...
**/
void PInstrument::handlePhi(Instruction *I, PHINode *PN, Function &F){
  Module *M = F.getParent();
  Type* Int64Ty = Type::getInt64Ty(M->getContext());
  IRBuilder<> IRB(I);
...
**/
Constant* PInstrument::handleOperand(Instruction *I, Value* OP, Function &F, 
...
**/
void PInstrument::handleIns(Function &F){
	for (Function::arg_iterator ait = F.arg_begin(), aend = F.arg_end();
                ait != aend; ++ait) {
  		InsCount++; 
	} 
...
/*
void PInstrument::handleExtractValue(Instruction *I, ExtractValueInst *EVI, Function &F){
  IRBuilder<> IRB(I);
  Module *M = F.getParent();
  Type* DoubleTy = Type::getDoubleTy(M->getContext());
...
*/
void PInstrument::handleFloatToInt(Instruction *I, BasicBlock *BB, FPToSIInst *FSI, Function &F){
  Instruction *Next = getNextInstruction(I, BB);
  IRBuilder<> IRB(Next);
  Module *M = F.getParent();
...
void PInstrument::handleSelect(Instruction *I, BasicBlock *BB, SelectInst *SI, Function &F){
...
**/
#if 1
void PInstrument::handleFcmp(Instruction *I, BasicBlock *BB, FCmpInst *FCI, Function &F){
  Instruction *Next = getNextInstruction(I, BB);
  IRBuilder<> IRB(Next);
  Module *M = F.getParent();
...
void PInstrument::handleMath(size_t Op, Instruction *I, BasicBlock *BB, CallInst *CI, Function &F){
  Instruction *Next = getNextInstruction(I, BB);
  IRBuilder<> IRB(Next);
  Module *M = F.getParent();
  ...
  Type* PtrVoidTy = PointerType::getUnqual(Type::getInt8Ty(M->getContext()));
	...
	Value *OP1Addr = CI->getArgOperand(0);


	Value *LAddr;
	BitCastInst* LDest = NULL;	
  if (LoadInst *Load = dyn_cast<LoadInst>(Next)){
  	LAddr = Load->getPointerOperand();
  	LDest = new BitCastInst(LAddr, PointerType::getUnqual(Type::getInt8Ty(M->getContext())),"", Next);
	}

  size_t InsIndex;
  InsIndex = InsMap.at(I);
  Constant* ConsInsIndex = ConstantInt::get(Type::getInt64Ty(M->getContext()), InsIndex); 
  Constant* OpCode = ConstantInt::get(Type::getInt64Ty(M->getContext()), Op);
  BitCastInst* Index1 = new BitCastInst(OP1Addr, PointerType::getUnqual(Type::getInt8Ty(M->getContext())),"", I);
	HandleOp = M->getOrInsertFunction("__compute_d", VoidTy, Int64Ty, PtrVoidTy, PtrVoidTy, Int64Ty, PtrVoidTy, Int64Ty);
	Value *Dest;
	Instruction *StoreIns;
	for (Use &U : CI->uses()) {
    User *UR = U.getUser();
		if (StoreInst *Store = dyn_cast<StoreInst>(UR)){
			Value *Inst1 = Store->getOperand(0);
			Value *Inst2 = Store->getOperand(1);
			Value *GOp = dyn_cast<GetElementPtrInst>(Inst2)->getPointerOperand();
			errs()<<"Inst1:"<<*Inst1<<"\n";
			errs()<<"Inst2:"<<*Inst2<<"\n";
			errs()<<"Gop:"<<*GOp<<"\n";
    	Value *Addr = Store->getPointerOperand();
...
//			Dest = IRBB.CreatePointerCast(Addr, Type::getInt64Ty(M->getContext())); 
  		BitCastInst* Dest = new BitCastInst(GOp, PointerType::getUnqual(Type::getInt8Ty(M->getContext())),"", StoreIns);
			errs()<<"Dest:"<<*Dest<<"\n";
			IRBB.CreateCall(HandleOp, {OpCode, Index1, Index1, CI, Dest, ConsInsIndex});
		}
	}
}

void PInstrument::handleAdd(size_t Op, Instruction *I, BasicBlock *BB, CallInst *CI, Function &F){
  Instruction *Next = getNextInstruction(I, BB);
  IRBuilder<> IRB(Next);
  Module *M = F.getParent();
  ...
  Type* PtrVoidTy = PointerType::getUnqual(Type::getInt8Ty(M->getContext()));
	...
	Value *OP1Addr = CI->getArgOperand(0);
	Value *OP2Addr = CI->getArgOperand(1);
  ...
  Constant* ConsInsIndex = ConstantInt::get(Type::getInt64Ty(M->getContext()), InsIndex); 
  Constant* OpCode = ConstantInt::get(Type::getInt64Ty(M->getContext()), Op);
//	Value *Index1 = IRB.CreatePointerCast(OP1Addr, Type::getInt64Ty(M->getContext())); 
//	Value *Index2 = IRB.CreatePointerCast(OP2Addr, Type::getInt64Ty(M->getContext())); 
  BitCastInst* Index1 = new BitCastInst(OP1Addr, PointerType::getUnqual(Type::getInt8Ty(M->getContext())),"", I);
  BitCastInst* Index2 = new BitCastInst(OP2Addr, PointerType::getUnqual(Type::getInt8Ty(M->getContext())),"", I);
	HandleOp = M->getOrInsertFunction("__compute_d", VoidTy, Int64Ty, PtrVoidTy, PtrVoidTy, Int64Ty, PtrVoidTy, Int64Ty);
	Value *Dest;
	Instruction *StoreIns;
	for (Use &U : CI->uses()) {
...
//			Dest = IRBB.CreatePointerCast(Addr, Type::getInt64Ty(M->getContext())); 
  		BitCastInst* Dest = new BitCastInst(GOp, PointerType::getUnqual(Type::getInt8Ty(M->getContext())),"", StoreIns);
			errs()<<"Dest:"<<*Dest<<"\n";
			IRBB.CreateCall(HandleOp, {OpCode, Index1, Index2, CI, Dest, ConsInsIndex});
		}
	}
}
/**
...
**/
void PInstrument::handleOp(Instruction *I, BasicBlock *BB, BinaryOperator* BO, Function &F){
  Instruction *Next = getNextInstruction(I, BB);
	Instruction *StoreIns;
  IRBuilder<> IRB(Next);
  Module *M = F.getParent();
...
/*
void PInstrument::handleOp(BasicBlock *BB, Function &F){
  Module *M = F.getParent();
  Type *Int1Ty = Type::getInt1Ty(M->getContext());
  Type *Int64Ty = Type::getInt64Ty(M->getContext());
  Type* VoidTy = Type::getVoidTy(M->getContext());
...
char PInstrument::ID = 0;
static const RegisterPass<PInstrument> Y("pos", "instrument fp operations", false, false);

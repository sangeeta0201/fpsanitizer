//===-llvm/lib/Transforms/PInstrument.h  - Interface ----------*- C++ -*-===//
//
//
//
//===----------------------------------------------------------------------===//
...
namespace {
struct PInstrument : public ModulePass {
  
public:
  PInstrument() : ModulePass(ID) {}
  ...
  Instruction* getNextInstruction(Instruction *I, BasicBlock *BB);
	void handleAdd(size_t OpCode, Instruction *I, BasicBlock *BB, CallInst *CI, Function &F);
	void handleMath(size_t Op, Instruction *I, BasicBlock *BB, CallInst *CI, Function &F);
